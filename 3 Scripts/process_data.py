...
from merge_data import make_sheets, merge_sheets
...
def extract_pfss(company_name, style, dev_desc, brand):
    ...
    :type style: str
    :param dev_desc: Breast implant deviceBescription
    :type dev_desc: str
...
def extract_pfss_allergan(style):
    styles_dict = literal_eval(open("3 Scripts/natrelle_style_dict.txt").read())
    info = {
        "profile": "",
        "filling": "",
        "shape"  : "",
...
def extract_pfss_sientra(style):
    styles_dict = literal_eval(open("3 Scripts/sientra_style_dict.txt").read())
    info = {
        "profile": "",
        "filling": "",
        "shape"  : "",
...
def assign_company_device_name(brand_name, shell_surface, filling, shape):
    companyname_dict = {
        ...
        'natrelle inspira softtouch'    : {'smooth': 'NATRELLE SoftTouch Smooth Round Breast Implant',
                                           'textured': 'NATRELLE SoftTouch Textured Breast Implant'}, 
        'natrelle inspira'              : {'smooth': 'NATRELLE Responsive Smooth Round Breast Implant',
                                           'textured': 'NATRELLE Responsive Textured Breast Implant'},
...
def assign_dimensions(cn, catnum, pdi):
    ...
    if cn == "MENTOR TEXAS L.P.":
        ...
        ddict = literal_eval(open("2 Data/allergan_catalog_dict.txt").read())
    elif cn == "Sientra, Inc.":
...
def process_data(dataframe):
    ...
    # df = df.assign(**{col: [dct[col] for dct in dimension_dicts] for col in step5_cols})
...
def main():
    gudid_folder="2 Data/AccessGUDID_6-28-2018"
