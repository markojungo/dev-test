...
def reference_dictionary(compname):
    if compname == "mentor":
        df = pd.read_csv("4 Product Catalogs/MENTOR catalog.csv")
    elif compname == "allergan":
        df = pd.read_csv("4 Product Catalogs/ALLERGAN NATRELLE catalog.csv")
    elif compname == "sientra":
        df = pd.read_csv("4 Product Catalogs/SIENTRA catalog.csv")
    else: # compname == "IDEAL IMPLANT INCORPORATED":
        df = pd.read_csv("4 Product Catalogs/IDEAL catalog.csv")    
